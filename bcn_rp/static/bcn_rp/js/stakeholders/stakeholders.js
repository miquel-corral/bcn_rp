...
var svg, tooltip, biHiSankey, path, defs, colorScale, highlightColorScale, isTransitioning, stakeholdersData;
...
function getTypeName(typeId){
    return stakeholdersData.dependencyTypes[typeId];
}
...
function getGraphData(id){
    ...
    if(typeof(id)==='undefined'){
        url="/dependencies_list";
    }else{
        url="/dependencies_list?assesstmentElementId=" + id + "&level=2";
    }

    d3.json(url, function(error, data) {
        if (error) return console.error('Error loading graph data', error)
        var graphData = data;
        getTypeData(graphData);
    });
 }

 function getTypeData(graphData){
    d3.json("/dependency_type_list", function(error, data) {
        if (error) return console.error('Error loading type data', error)
        var typeData = data;
        stakeholdersData = new StakeholdersData(graphData, typeData);
        createGraph(stakeholdersData);

    });
 }


function createGraph(stakeholdersData){

biHiSankey
  .nodes(stakeholdersData.nodes)
  .links(stakeholdersData.links)
